...
bool checkedges(const PolygonalMesh& mesh, const double& epsilon)
{
    ...
    for(size_t i = 0; i<mesh.VerticesCell1d.size(); ++i)
    {
        ...
        if (length < 2*(sqrt(epsilon))/(pow(3,1/4))) // dato che l'area del poligono più piccolo è un triangolo equilatero di lato L,
        // se impongo che l'area sia maggiore di epsilon devo controllare che il lato non sia minore di L = 3^(1/4)(epsilon)^(1/2)
        {
...
bool checkareas(PolygonalMesh& mesh, const double& epsilon)
{
    bool check = true; //bool di controllo
    double x1;
    double x2;
    double y1;
    double y2;
    for (unsigned int i = 0; i < mesh.NumberCell2d; ++i)
    {
        ...
        if (area < epsilon)
        {
            //area del poligono più piccolo: triangolo equilatero di lato 10*epsilon
            cout << "Il poligono di id: " << mesh.IdCell2d[i] << " ha area nulla" << endl;
            check = false;
        }
    }
    if (check)
...
bool checkMarkers0d(const PolygonalMesh& mesh, const double& epsilon)
{
    bool check = true;
    ...
    for (const auto& markerPair : mesh.Cell0dMarkers)
    {
        ...
        for (unsigned int id : ids)
        {
            ...
            if (it!=mesh.IdCell0d.end())
            {
                ...
                {
                    if (x < epsilon || abs(x - 1) < epsilon ||
                        y < epsilon || abs(y - 1) < epsilon)
                    {
                ...
                {
                    if (!(x < epsilon || abs(x - 1) < epsilon ||
                          y < epsilon || abs(y - 1) < epsilon)) // se il marker è 0, i punti non devono essere sul bordo
                    {                       
